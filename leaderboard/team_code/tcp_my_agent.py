...
from TCP.model_transformer import VitFuser
from TCP.config import GlobalConfig
from TCP.data_new import scale_and_crop_image, get_left_right_img
...
def get_entry_point():
    return 'VitFuserAgent'


class VitFuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        self.net = VitFuser(self.config, "cuda")
    ...
    def run_step(self, input_data, timestamp):
        ...
        command = torch.tensor(command).unsqueeze(0).to('cuda', dtype=torch.int)
        ...
        front_img = scale_and_crop_image(img, scale=1.0, crop_w=256, crop_h=256)
        focus_img = scale_and_crop_image(img, scale=0.35, crop_w=256, crop_h=256)
        front_img = self._im_transform(np.array(front_img)).unsqueeze(0).to('cuda', dtype=torch.float32)
        focus_img = self._im_transform(np.array(focus_img)).unsqueeze(0).to('cuda', dtype=torch.float32)
        left_img, right_img = get_left_right_img(img)
        left_img = self._im_transform(np.array(left_img)).unsqueeze(0).to('cuda', dtype=torch.float32)
        right_img = self._im_transform(np.array(right_img)).unsqueeze(0).to('cuda', dtype=torch.float32)
