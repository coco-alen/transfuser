...
class TCP_planner(pl.LightningModule):
    def __init__(self, config, lr):
        ...
        self.lr = lr
        self.config = config
        self.model = VitFuser(config, device='cuda')
        self._load_weight()
    ...
    def training_step(self, batch, batch_idx):
        ...
        focus_img = batch['focus_img']
        left_img = batch['left_img']
        right_img = batch['right_img']
        ...
        feature_loss = F.mse_loss(pred['pred_feature'], feature) * self.config.features_weight
        ...
        loss = action_loss + speed_loss + feature_loss + wp_loss+ future_feature_loss + future_action_loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log("batch_throttle_l1", batch_throttle_l1, sync_dist=True)
        self.log('batch_steer_l1', batch_steer_l1, sync_dist=True)
        self.log('batch_brake_l1', batch_brake_l1, sync_dist=True)
