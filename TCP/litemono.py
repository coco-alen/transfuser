...
class LiteMono(nn.Module):
    ...
    def __init__(self, in_chans=3, model='lite-mono', height=192, width=640,
        ...
        self.downsample = nn.Conv2d(128, 512, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        x_feauture = self.downsample(x[-1])
        x = nn.functional.adaptive_avg_pool2d(x_feauture, (1, 1))
        x = torch.flatten(x, 1)
        out = self.classifier(x)

        return out, x_feauture

if __name__ == '__main__':
    model = LiteMono(model='lite-mono-tiny', height=256, width=900)
    input = torch.randn(1, 3, 256, 900)
    output = model(input)
    print(output[0].size())
    print(output[1].size())