...
class CARLA_Data(Dataset):
	...
	def __getitem__(self, index):
		...
		data = dict()
		img = Image.open(self.root+self.front_img[index][0])
		front_img = scale_and_crop_image(img, scale=1, crop_w=256, crop_h=256)
		focue_img = scale_and_crop_image(img, scale=0.35, crop_w=256, crop_h=256)
		left_img, right_img = get_left_right_img(img)
		if self.img_aug:
			data['front_img'] = self._im_transform(augmenter(self._batch_read_number).augment_image(np.array(front_img)))
			data['focus_img'] = self._im_transform(augmenter(self._batch_read_number).augment_image(np.array(focue_img)))
			data['left_img'] = self._im_transform(augmenter(self._batch_read_number).augment_image(np.array(left_img)))
			data['right_img'] = self._im_transform(augmenter(self._batch_read_number).augment_image(np.array(right_img)))
		else:
			data['front_img'] = self._im_transform(np.array(front_img))
			data['focus_img'] = self._im_transform(np.array(focue_img))
			data['left_img'] = self._im_transform(np.array(left_img))
			data['right_img'] = self._im_transform(np.array(right_img))
...
def get_left_right_img(image):
	(width, height) = (int(image.width), int(image.height))
	assert height == 256, "height should be 256"
	start_y = width//2 - 256 - 128
	end_y = width//2 - 128
	left_img = image.crop((start_y, 0, end_y, 256))

	start_y = width//2 + 128
	end_y = width//2 + 256 + 128
	right_img = image.crop((start_y, 0, end_y, 256))
